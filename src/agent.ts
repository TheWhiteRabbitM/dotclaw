...
export class DOTClawAgent {
  ...
  private async detectIntent(message: string): Promise<any> {
    ...
    if (lower.includes('balance')) return { action: 'balance' };
    if (lower.includes('validator')) return { action: 'validators' };
    if (lower.includes('pool') && !lower.includes('join')) return { action: 'pools' };
    
    // Stake X DOT
    const stakeMatch = lower.match(/stake\s+(\d+)/);
    if (stakeMatch) return { action: 'stake', amount: stakeMatch[1] };
    
    // Join pool X with Y DOT
    const joinMatch = lower.match(/join\s+pool\s+(\d+)\s+(?:with\s+)?(\d+)/);
    if (joinMatch) return { action: 'join_pool', poolId: parseInt(joinMatch[1]), amount: joinMatch[2] };
    
    // Unbond X DOT
    const unbondMatch = lower.match(/unbond\s+(\d+)/);
    if (unbondMatch) return { action: 'unbond', amount: unbondMatch[1] };
    
    // Swap X for Y
    const swapMatch = lower.match(/swap\s+(\d+)\s+(\w+)\s+for\s+(\w+)/);
    if (swapMatch) return { action: 'swap', amount: swapMatch[1], from: swapMatch[2], to: swapMatch[3] };
